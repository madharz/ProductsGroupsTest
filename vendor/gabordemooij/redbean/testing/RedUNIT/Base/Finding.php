...
class Finding extends Base {
	...
	public function testFindMultiWithSQLNULL()
	{
		...
		$products = R::dispense( 'products', 3 );
		...
		$collection = R::findMulti( 'shop,products,price', NULL, array(), array(
			'0' => Finder::map( 'shop', 'products' ),
			'1' => Finder::map( 'products', 'price' ),
		));
		asrt( is_array( $collection ), TRUE );
		asrt( count( $collection ), 3 );
		asrt( count( $collection['shop'] ), 2 );
		asrt( count( $collection['products'] ), 3 );
	...
	public function testFindMultiExtFunc()
	{
		...
		$shop2->ownProduct[] = $products[2];
		R::storeAll( array( $shop, $shop2 ) );
		$collection = R::findMulti( 'shop,products,price', '
			SELECT shop.*, products.*, price.* FROM shop
			LEFT JOIN products ON products.shop_id = shop.id
	...
	{
		R::nuke();
		$collection = R::findMulti( 'shop,products', array(
		...
		asrt( isset( $collection['products'] ), TRUE );
		asrt( (int) $collection['shop'][1]->id, 1 );
		asrt( (int) $collection['products'][1]->id, 1 );
		asrt( (int) $collection['products'][2]->id, 2 );
		asrt( (int) $collection['products'][3]->id, 3 );
		asrt( (int) $collection['products'][1]->shopID, 1 );
		asrt( (int) $collection['products'][2]->shopID, 1 );
		asrt( (int) $collection['products'][3]->shopID, 1 );
		asrt( $collection['products'][1]->name, 'vase' );
		asrt( $collection['products'][2]->name, 'candle' );
		asrt( $collection['products'][3]->name, 'plate' );
		R::nuke();
		$shop = R::dispense('shop');
		$shop2 = R::dispense('shop');
		$products = R::dispense('products', 3);
